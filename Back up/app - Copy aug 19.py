...
<head>
    ...
    <style>
        body { 
            ...
            flex-direction: column; /* Changed to column to place header at top */
            min-height: 100vh;
        }
        .global-header { /* New global header styling */
        ...
        .main-content-flex-container { /* New container to hold side-nav and main-content-area */
            display: flex;
            flex-grow: 1; /* Allows this container to take remaining space */
            width: 100%; /* Ensure it spans full width below header */
        }
        .login-register-container { /* Explicit container for auth forms */
            flex-grow: 1;
            padding: 20px;
            display: flex; /* To center forms */
            justify-content: center;
            align-items: center;
        }
        .main-content { /* Existing main content for collection display */
            flex-grow: 1;
            padding: 20px;
        }
        .container {
    ...
    <script>
        ...
        let loggedInUser = '{{ logged_in_user }}'; // Get from Flask template

        // Convert the string 'None' or empty string '' from Flask template to actual null
        if (loggedInUser === 'None' || loggedInUser === '') {
            loggedInUser = null;
        }
        ...
        async function updateUI(loggedInUserParam, displayedUserParam) {
            ...
            const mainContentFlexContainer = document.getElementById('main-content-flex-container'); // Renamed ID
            ...
            if (loggedInUser) {
                ...
                loginRegisterContainer.style.display = 'none'; // Hide auth forms
                mainContentFlexContainer.style.display = 'flex'; // Show main app content (side nav + collection)
                ...
                loginRegisterContainer.style.display = 'flex'; // Show auth forms container (flex for centering)
                loginSection.style.display = 'block'; // Default to showing login form
                registerSection.style.display = 'none'; // Hide register form initially
                mainContentFlexContainer.style.display = 'none'; // Hide main app content
...
<body>
    <div class="global-header"> <!-- Global header, always visible -->
        <div class="container">
            <div class="header">
                <h1>{% if current_displayed_user %}{{ current_displayed_user }}'s Card Collection{% else %}My Card Collection{% endif %}</h1>
    ...
    <div class="main-container-flex"> <!-- New flex container for side-nav and main-content-area -->
        ...
        <div class="login-register-container" id="login-register-container" style="display: flex;"> <!-- Changed to flex by default -->
            <div class="container">
